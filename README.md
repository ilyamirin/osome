...
Игрок видит поле `4x5` с очередью клиентов и текущую цветную коробку на стойке. Нужно тапать по клиенту того же цвета. Если рядом есть связанная группа такого же цвета, она исчезает целиком. Ошибка сбрасывает комбо и ускоряет рост напряжения. Правильные действия приносят очки, комбо, временные бонусы и усиливают темп игры. По мере сессии растут базовая сложность, частота гиммиков и адаптивная скорость спавна.
...
- Связанные группы `3+` одного цвета убираются одним тапом
...
5. Если попал в связанную группу `3+` того же цвета, исчезнут все смежные клиенты без брешей.
...
- Если у клиента есть связанная группа того же цвета по сторонам, группа `3+` уходит целиком.
...
- Связка `3+`: дополнительные очки за размер группы
